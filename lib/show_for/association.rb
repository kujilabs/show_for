module ShowFor
  ...
  protected
    ...
    def values_from_association(association, options) #:nodoc:
      sample = association.is_a?(Array) || association.is_a?(ActiveRecord::Associations::CollectionProxy) ? association.first : association
      ...
      if method
        association.is_a?(Array) || association.is_a?(ActiveRecord::Associations::CollectionProxy) ? association.map(&method) : association.try(method)
      end
    end
  end
end
